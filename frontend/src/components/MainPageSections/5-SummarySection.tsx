...
function SummarySection() {
  ...
  return (
    <div className="p-4 bg-zinc-700 rounded-lg shadow-md mb-2">
      ...
      <p className="mt-2"></p>
      <p className="text-base font-bold mb-2">
        🟢<strong>PREVENÇÕES E SEGUIMENTOS</strong>
      </p>
      {/* Exibe o risco cardiovascular */}
      {CVRRealRisk !== undefined && (
        <li>
          {" "}
          <strong>RCV:</strong> Risco Atual: {CVRRealRisk.toFixed(2)}% - (
          {CVRcategory}) || Risco Ideal: {CVRIdealRisk.toFixed(2)}%{" "}
        </li>
      )}
      {/* Exibe a TFG se houver */}
      {eGFR !== undefined && (
        <li>
          <strong>TFG: </strong> {eGFR} mL/min/1.73m²
        </li>
      )}
      ...
      {fib4 !== undefined && (
        <li>
          <strong>FIB4: </strong>
          {fib4} pontos - Estadiamento de Fibrose estimado: {fib4category}{" "}
        </li>
      )}
    </div>
  );
